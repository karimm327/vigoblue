...
// ----------------- Routes statiques -----------------
app.get("/", (req, res) =>
  res.sendFile(path.join(__dirname, "vigoblue/login.html"))
);
app.get("/signup.html", (req, res) =>
  res.sendFile(path.join(__dirname, "vigoblue/signup.html"))
);
app.get("/reset-password.html", (req, res) =>
  res.sendFile(path.join(__dirname, "vigoblue/reset-password.html"))
);
...
app.post("/send-verification-code", async (req, res) => {
  ...
  if (!isGmail(email))
    return res.status(400).json({
      success: false,
      message: "Seuls les emails Gmail sont autorisés !",
    });
  ...
  try {
    ...
    try {
      ...
      return res
        .status(500)
        .json({ success: false, message: "Erreur base de données" });
    }
